...
class PreProcessor:
    ...
    def __init__(self, networkFile, sumocfgFile, hasGUI: bool, logger: Logger, name: str, args, testing=False):
        ...
        self.dir = "/".join(sumocfgFile.split("/")[0:-1] + ["solutions", self.__name]) if not testing else "/".join(["solutions", self.__name])
        os.makedirs(self.dir, exist_ok=True)
        self.timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        self.experimentRadix = f"{self.dir}/{self.timestamp}"
    ...
    def solve(self, fromFile=None):
        ...
        while step <= self.__HORIZON:
            ...
            countVehiclesOnEdge = 0
            countVehiclesOnStreet = 0
            edgeToCheckOccupancy = "189[1][1]"
            for v in vehiclesInside:
                v.routeIndex
                idEdge = traci.vehicle.getRoadID(v.id)
                edgesRoute = traci.vehicle.getRoute(v.id)
                edgeVeh = idEdge
                v.getFirstStreet()
                if edgeToCheckOccupancy == edgeVeh:
                    countVehiclesOnEdge += 1
                s = v.getFirstStreet()
                streetsString = [str(st) for st in s.getOriginalStreets()]
                if edgeToCheckOccupancy in streetsString:
                    countVehiclesOnStreet += 1

            self.logger.log(f"there are {countVehiclesOnEdge} vehicles on edge, {countVehiclesOnStreet} vehicles on street [{edgeToCheckOccupancy}]")
