...
class RunsDB:
    ...
    def buildDB(self, dbName="results.db", deletePrior=True):
        ...
        self.cursor.execute(
            'CREATE TABLE result (runID integer, denominator integer, key text, value plain)')
        self.cursor.execute(
            'CREATE TABLE timed_result (runID integer, denominator integer, aib plain, aie plain, key text, value plain)')
        self.conn.commit()
        self.run = 0
