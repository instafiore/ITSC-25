...
class Problem:
    ...
    def __init__(self, sim: Simulation, vehiclesInside: [Vehicle], newVehicles: [Vehicle], prevSolution: Solution,
                 problemType=ProblemType.COMPUTE_OPTIMUM, optimumToBeat=None, emissionMap: Dict[Tuple[str], float] =dict(), step = 0, logger = None):
        self.sim = sim
    ...
    def __addEmissionMap(self):
        if self.emissionMap:
            for key in self.emissionMap:
                ...
                emissionsInGrams = int(self.emissionMap[key])
